...
local t = json:decode(https.request('http://klanrplay.ml/REMIX/angel.php?id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write")))
print("::REMIX::")
local RunBot = io.open("REMIX", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/REMIX
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr REMIX.lua
wget "https://raw.githubusercontent.com/REMIX-TEAM/REMIX/master/REMIX.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./REMIX.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S REMIX -X kill
screen -S REMIX ./REMIX
done
]])
RunTs:close()
end
